...
function onSubmit(e) {
    e.preventDefault();
    for(let i = 0; i <= amount; i++)createPromise(i, delay + step * i).then((data)=>{
        console.log(data);
    }).catch((error)=>{
        console.error(error);
    });
...
function createPromise(position, delay1) {
    ...
    return new Promise((resolve, reject)=>{
        setTimeout(()=>{
            if (shouldResolve) resolve(`✅ Fulfilled promise ${position} in ${delay1}ms`);
            else reject(`❌ Rejected promise ${position} in ${delay1}ms`);
        }, delay1);
    });
